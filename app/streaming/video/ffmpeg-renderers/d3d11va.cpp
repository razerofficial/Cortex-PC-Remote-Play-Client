...
bool D3D11VARenderer::initialize(PDECODER_PARAMETERS params)
{
    ...
    char* envValue = std::getenv("D3D11VA_ENABLED");
    if(envValue && strcmp(envValue, "0") == 0) {
        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION,
...
bool D3D11VARenderer::setupRenderingResources()
{
    ...
    {
        std::string vertexShaderBytecode = Path::readDataFile("d3d11_vertex.fxc");

        ID3D11VertexShader* vertexShader;
        hr = m_Device->CreateVertexShader(vertexShaderBytecode.data(), vertexShaderBytecode.length(), nullptr, &vertexShader);
        if (SUCCEEDED(hr)) {
        ...
        hr = m_Device->CreateInputLayout(vertexDesc, ARRAYSIZE(vertexDesc), vertexShaderBytecode.data(), vertexShaderBytecode.length(), &inputLayout);
        if (SUCCEEDED(hr)) {
    ...
    {
        std::string overlayPixelShaderBytecode = Path::readDataFile("d3d11_overlay_pixel.fxc");

        hr = m_Device->CreatePixelShader(overlayPixelShaderBytecode.data(), overlayPixelShaderBytecode.length(), nullptr, &m_OverlayPixelShader);
        if (FAILED(hr)) {
            SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
    ...
    {
        std::string videoPixelShaderBytecode = Path::readDataFile("d3d11_genyuv_pixel.fxc");

        hr = m_Device->CreatePixelShader(videoPixelShaderBytecode.data(), videoPixelShaderBytecode.length(), nullptr, &m_VideoGenericPixelShader);
        if (FAILED(hr)) {
            SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                         "ID3D11Device::CreatePixelShader() failed: %x",
                         hr);
    ...
    {
        std::string videoPixelShaderBytecode = Path::readDataFile("d3d11_bt601lim_pixel.fxc");

        hr = m_Device->CreatePixelShader(videoPixelShaderBytecode.data(), videoPixelShaderBytecode.length(), nullptr, &m_VideoBt601LimPixelShader);
        if (FAILED(hr)) {
            SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                         "ID3D11Device::CreatePixelShader() failed: %x",
                         hr);
    ...
    {
        std::string videoPixelShaderBytecode = Path::readDataFile("d3d11_bt2020lim_pixel.fxc");

        hr = m_Device->CreatePixelShader(videoPixelShaderBytecode.data(), videoPixelShaderBytecode.length(), nullptr, &m_VideoBt2020LimPixelShader);
        if (FAILED(hr)) {
            SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
                         "ID3D11Device::CreatePixelShader() failed: %x",
                         hr);
