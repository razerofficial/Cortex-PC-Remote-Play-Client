...
bool DXVA2Renderer::initializeQuirksForAdapter(IDirect3D9Ex* d3d9ex, int adapterIndex)
{
...

    {
        ...
        m_DeviceQuirks = Environment::environmentVariableIntValue("DXVA2_QUIRK_FLAGS", &ok);
        if (ok) {
            SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION,
...
bool DXVA2Renderer::isDecoderBlacklisted()
{
    ...
    char* envValue = std::getenv("DXVA2_DISABLE_DECODER_BLACKLIST");
...
{
    char* envValue = std::getenv("DXVA2_ENABLED");
...
//#ifndef Q_PROCESSOR_X86
    else if (envValue && strcmp(envValue, "1") != 0 && m_DecoderSelectionPass == 0) {
        SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION,
