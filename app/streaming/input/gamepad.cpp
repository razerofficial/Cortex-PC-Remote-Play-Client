...
Uint32 SdlInputHandler::mouseEmulationTimerCallback(Uint32 interval, void *param)
{
    ...
    deltaX = std::pow(rawX / 32766.0f * MOUSE_EMULATION_MOTION_MULTIPLIER, 3);
    deltaY = std::pow(rawY / 32766.0f * MOUSE_EMULATION_MOTION_MULTIPLIER, 3);

    // Enforce deadzones
    deltaX = std::fabs(deltaX) > MOUSE_EMULATION_DEADZONE ? deltaX - MOUSE_EMULATION_DEADZONE : 0;
    deltaY = std::fabs(deltaY) > MOUSE_EMULATION_DEADZONE ? deltaY - MOUSE_EMULATION_DEADZONE : 0;
    ...
    for (;;) {
        switch (event->axis)
        {
            ...
            case SDL_CONTROLLER_AXIS_LEFTY:
                ...
                state->lsY = -(std::max)(event->value, (short)-32767);
                break;
            case SDL_CONTROLLER_AXIS_RIGHTX:
                state->rsX = event->value;
                break;
            case SDL_CONTROLLER_AXIS_RIGHTY:
                state->rsY = -(std::max)(event->value, (short)-32767);
                break;
...
void SdlInputHandler::handleControllerButtonEvent(SDL_ControllerButtonEvent* event)
{
    ...
    // Handle Start+Select as a gamepad quit combo
...
void SdlInputHandler::handleControllerDeviceEvent(SDL_ControllerDeviceEvent* event)
{
    ...
    if (event->type == SDL_CONTROLLERDEVICEADDED) {
        ...
        if(m_IgnoreDeviceGuids.end() != std::find_if(m_IgnoreDeviceGuids.begin(), m_IgnoreDeviceGuids.end(),
                                                      [&](const std::string& str) { return StringUtils::caseInsensitiveCompare(str, guidStr); }))
        {
            SDL_LogInfo(SDL_LOG_CATEGORY_APPLICATION,
...
std::string SdlInputHandler::getUnmappedGamepads()
{
    std::string ret;
