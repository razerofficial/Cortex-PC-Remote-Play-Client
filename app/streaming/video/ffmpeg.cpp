...
static const std::map<std::string, int> k_NonHwaccelCodecInfo = {
    // H.264
    {"h264_mmal", 0},
    {"h264_rkmpp", 0},
    {"h264_nvv4l2", 0},
...
int FFmpegVideoDecoder::getDecoderCapabilities()
{
    ...
    int capabilities = Environment::environmentVariableIntValue("DECODER_CAPS", &ok);
    if (ok) {
        ...
        if (!isHardwareAccelerated()) {
            ...
            int slices = (std::min)(MAX_SLICES, SDL_GetCPUCount());
            ...
            if(k_NonHwaccelCodecInfo.end() != k_NonHwaccelCodecInfo.find(m_VideoDecoderCtx->codec->name))
            {
                capabilities = k_NonHwaccelCodecInfo.at(m_VideoDecoderCtx->codec->name);
            }
            else
            {
                capabilities = 0;
...
RazerSize FFmpegVideoDecoder::getDecoderMaxResolution()
{
    if (m_BackendRenderer->getRendererAttributes() & RENDERER_ATTRIBUTE_1080P_MAX) {
        return RazerSize(1920, 1080);
    }
    else {
        // No known maximum
        return RazerSize(0, 0);
    }
}
...
bool FFmpegVideoDecoder::completeInitialization(const AVCodec* decoder, enum AVPixelFormat requiredFormat, PDECODER_PARAMETERS params, bool testFrame, bool useAlternateFrontend)
{
    ...
    if (!isHardwareAccelerated()) {
        ...
        m_VideoDecoderCtx->thread_count = (std::min)(MAX_SLICES, SDL_GetCPUCount());
    }
    else {
...
bool FFmpegVideoDecoder::initialize(PDECODER_PARAMETERS params)
{
    ...
    // - AV_PIX_FMT_NV21
    {
        char* envValue = std::getenv("H264_DECODER_HINT");
        std::string h264DecoderHint;
        if(nullptr != envValue)
            h264DecoderHint = envValue;
        if (!h264DecoderHint.empty() && (params->videoFormat & VIDEO_FORMAT_MASK_H264)) {
            //QByteArray decoderString = h264DecoderHint.toLocal8Bit();
            if (tryInitializeRendererForUnknownDecoder(avcodec_find_decoder_by_name(h264DecoderHint.data()), params, true)) {
                SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION,
                            "Using custom H.264 decoder (H264_DECODER_HINT): %s",
                            h264DecoderHint.data());
                return true;
            }
            else {
                SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
    ...
    {
        char* envValue = std::getenv("HEVC_DECODER_HINT");
        std::string hevcDecoderHint;
        if(nullptr != envValue)
            hevcDecoderHint = envValue;
        if (!hevcDecoderHint.empty() && (params->videoFormat & VIDEO_FORMAT_MASK_H265)) {
            //QByteArray decoderString = hevcDecoderHint.toLocal8Bit();
            if (tryInitializeRendererForUnknownDecoder(avcodec_find_decoder_by_name(hevcDecoderHint.data()), params, true)) {
                SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION,
                            "Using custom HEVC decoder (HEVC_DECODER_HINT): %s",
                            hevcDecoderHint.data());
                return true;
            }
            else {
                SDL_LogError(SDL_LOG_CATEGORY_APPLICATION,
    ...
    {
        char* envValue = std::getenv("AV1_DECODER_HINT");
        std::string av1DecoderHint;
        if(nullptr != envValue)
            av1DecoderHint = envValue;
        if (!av1DecoderHint.empty() && (params->videoFormat & VIDEO_FORMAT_MASK_AV1)) {
            //QByteArray decoderString = av1DecoderHint.toLocal8Bit();
            if (tryInitializeRendererForUnknownDecoder(avcodec_find_decoder_by_name(av1DecoderHint.data()), params, true)) {
                SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION,
    ...
    if (params->vds != StreamingPreferences::VDS_FORCE_SOFTWARE) {
        ...
        while ((decoder = av_codec_iterate(&codecIterator))) {
            ...
            if (terminallyFailedHardwareDecoders.end() != terminallyFailedHardwareDecoders.find(decoder)) {
                continue;
            }
...
void FFmpegVideoDecoder::writeBuffer(PLENTRY entry, int& offset)
{
    if (m_NeedsSpsFixup && entry->bufferType == BUFFER_TYPE_SPS) {
        ...
        memcpy(const_cast<char *>(&m_DecodeBuffer.data()[initialOffset]), entry->data, nalStart);
        offset += nalStart;
        ...
        memcpy(const_cast<char *>(&m_DecodeBuffer.data()[offset]),
               entry->data,
               entry->length);
        offset += entry->length;
    }
            ...
            do {
                ...
                if (err == 0) {
                    ...
                    if (!m_FrameInfoQueue.empty()) {
                        ...
                        DECODE_UNIT du = m_FrameInfoQueue.front();
                        m_FrameInfoQueue.pop();
                    ...
                    SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION,
                                ...
                                errorstring,
                                !m_FrameInfoQueue.empty() ? m_FrameInfoQueue.front().frameNumber : -1);
...
int FFmpegVideoDecoder::submitDecodeUnit(PDECODE_UNIT du)
{
    ...
    m_Pkt->data = reinterpret_cast<uint8_t*>(const_cast<char*>(m_DecodeBuffer.data()));
    m_Pkt->size = offset;
