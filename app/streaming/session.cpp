...
bool Session::s_busy = false;
std::mutex Session::s_busyMutex;
Session::State Session::s_state = Session::NoError;
std::string Session::s_stageText;
std::string Session::s_streamErrorText;
Session* Session::s_ActiveSession;
RazerSemaphore Session::s_ActiveSessionSemaphore(1);
...
void Session::clStageStarting(int stage)
{
    ...
    oss << "Starting " << LiGetStageName(stage) << "...";
    s_stageText = oss.str();
}
...
void Session::clStageFailed(int stage, int errorCode)
{
    ...
    std::ostringstream oss;
    oss << "Starting " << LiGetStageName(stage) << " failed: Error " << errorCode;
    s_streamErrorText = oss.str();

    if (failingPorts)
        s_streamErrorText += std::string("\n\nCheck your firewall and port forwarding rules for port(s): ") + failingPorts;
}
...
void Session::clConnectionTerminated(int errorCode)
{
    ...
    case ML_ERROR_NO_VIDEO_TRAFFIC:
        ...
        oss << "No video received from host.\n\n" << "Check your firewall and port forwarding rules for port(s): " << ports;
        s_streamErrorText = oss.str();
        s_state = Session::AbnormalExit;
        break;

    case ML_ERROR_NO_VIDEO_FRAME:
        s_ActiveSession->m_UnexpectedTermination = true;
        oss << "Your network connection isn't performing well. Reduce your video bitrate setting or try a faster connection.";
        s_streamErrorText = oss.str();
        s_state = Session::AbnormalExit;
        break;
    ...
    case ML_ERROR_UNEXPECTED_EARLY_TERMINATION:
        s_ActiveSession->m_UnexpectedTermination = true;
        oss << "Something went wrong on your host PC when starting the stream.\n\n" << 
    ...
    case ML_ERROR_FRAME_CONVERSION:
        s_ActiveSession->m_UnexpectedTermination = true;
        oss << "The host PC reported a fatal video encoding error.\n\n" <<
    ...
    default:
        ...
        bool hexError = std::abs(errorCode) > 1000;
        oss << "Connection terminated\n\n" << "Error code: " << errorCode;
        s_streamErrorText = oss.str();
        s_state = Session::AbnormalExit;
        break;
    }
...
bool Session::populateDecoderProperties(SDL_Window* window)
{
    ...
    {
        bool ok;
        m_StreamConfig.colorSpace = Environment::environmentVariableIntValue("COLOR_SPACE_OVERRIDE", &ok);
        if (ok) {
        ...
        m_StreamConfig.colorRange = Environment::environmentVariableIntValue("COLOR_RANGE_OVERRIDE", &ok);
        if (ok) {
            SDL_LogWarn(SDL_LOG_CATEGORY_APPLICATION,
...
bool Session::initialize()
{
    ...
    LOG(INFO) << "Server GPU:" << m_Computer->gpuModel;
    LOG(INFO) << "Server GFE version:" << m_Computer->gfeVersion;
    ...
    if (m_Preferences->virtualDisplay)
    {
        m_StreamConfig.width = primaryMonitor.width;
        m_StreamConfig.height = primaryMonitor.height;
        m_StreamConfig.fps = primaryMonitor.refreshRate;
        m_StreamConfig.bitrate = StreamingPreferences::getDefaultBitrate(m_StreamConfig.width, m_StreamConfig.height, m_StreamConfig.fps);
    }
    else
    {
        m_StreamConfig.width = (0 == m_Preferences->width ? primaryMonitor.width : m_Preferences->width);
        m_StreamConfig.height = (0 == m_Preferences->height ? primaryMonitor.height : m_Preferences->height);
        m_StreamConfig.fps = (0 == m_Preferences->fps ? primaryMonitor.refreshRate : m_Preferences->fps);
        m_StreamConfig.bitrate = (0 == m_Preferences->bitrateKbps ? StreamingPreferences::getDefaultBitrate(m_StreamConfig.width, m_StreamConfig.height, m_StreamConfig.fps) : m_Preferences->bitrateKbps);
    }
...
void Session::emitLaunchWarning(std::string text)
{
...
bool Session::validateLaunch(SDL_Window* testWindow)
{
    if (!m_Computer->isSupportedServerVersion) {
        s_streamErrorText = "The version of RemotePlayHost is not supported!";
        return false;
    }
    ...
    if (m_StreamConfig.supportedVideoFormats & VIDEO_FORMAT_MASK_AV1) {
        if (!(m_Computer->serverCodecModeSupport & SCM_MASK_AV1)) {
            if (m_Preferences->videoCodecConfig == StreamingPreferences::VCC_FORCE_AV1) {
                emitLaunchWarning("Your host software or GPU doesn't support encoding AV1.");
            }
            ...
            m_StreamConfig.supportedVideoFormats &= ~VIDEO_FORMAT_MASK_AV1;
                 ...
                 !isHardwareDecodeAvailable(testWindow,
                                            ...
                                            m_StreamConfig.height,
                                            m_StreamConfig.fps)) {
            emitLaunchWarning("Using software decoding due to your selection to force AV1 without GPU support. This may cause poor streaming performance.");
        }
    }
    ...
    if (m_StreamConfig.supportedVideoFormats & VIDEO_FORMAT_MASK_H265) {
        if (m_Computer->maxLumaPixelsHEVC == 0) {
            if (m_Preferences->videoCodecConfig == StreamingPreferences::VCC_FORCE_HEVC) {
                emitLaunchWarning("Your host PC doesn't support encoding HEVC.");
            }
    ...
    if (!(m_StreamConfig.supportedVideoFormats & VIDEO_FORMAT_MASK_H265) &&
        ...
        if (m_Preferences->videoCodecConfig == StreamingPreferences::VCC_FORCE_H264) {
            emitLaunchWarning("Using software decoding due to your selection to force H.264 without GPU support. This may cause poor streaming performance.");
        }
        else {
            if (m_Computer->maxLumaPixelsHEVC == 0 &&
                    isHardwareDecodeAvailable(testWindow,
                                              ...
                                              m_StreamConfig.height,
                                              m_StreamConfig.fps)) {
                emitLaunchWarning("Your host PC and client PC don't support the same video codecs. This may cause poor streaming performance.");
            }
            else {
                emitLaunchWarning("Your client GPU doesn't support H.264 decoding. This may cause poor streaming performance.");
            }
        }
    }
    ...
    if (m_Preferences->enableHdr) {
        ...
        if (!(m_Computer->serverCodecModeSupport & SCM_MASK_10BIT)) {
            emitLaunchWarning("Your host PC doesn't support HDR streaming.");
            m_StreamConfig.supportedVideoFormats &= ~VIDEO_FORMAT_MASK_10BIT;
        }
        else if (m_Preferences->videoCodecConfig == StreamingPreferences::VCC_FORCE_H264) {
            emitLaunchWarning("HDR is not supported using the H.264 codec.");
            m_StreamConfig.supportedVideoFormats &= ~VIDEO_FORMAT_MASK_10BIT;
        }
            ...
            if ((m_Computer->serverCodecModeSupport & SCM_AV1_MAIN10) && (m_StreamConfig.supportedVideoFormats & VIDEO_FORMAT_AV1_MAIN10)) {
                if (!isHardwareDecodeAvailable(testWindow,
                                               ...
                                               m_StreamConfig.height,
                                               m_StreamConfig.fps)) {
                    emitLaunchWarning("This PC's GPU doesn't support AV1 Main10 decoding for HDR streaming.");
                    m_StreamConfig.supportedVideoFormats &= ~VIDEO_FORMAT_AV1_MAIN10;
                }
            }
            ...
            emitLaunchWarning("This PC's GPU doesn't support 10-bit HEVC or AV1 decoding for HDR streaming.");
        }
        ...
        if ((m_StreamConfig.supportedVideoFormats & VIDEO_FORMAT_MASK_10BIT) && // Ignore this check if we already failed one above
            ...
            emitLaunchWarning("Your host PC and client PC don't support the same HDR video codecs.");
            m_StreamConfig.supportedVideoFormats &= ~VIDEO_FORMAT_MASK_10BIT;
        }
    }
    ...
    if (m_StreamConfig.width >= 3840) {
        ...
        if (m_Computer->gfeVersion.empty() || WMUtils::startsWith(m_Computer->gfeVersion, "2.")) {
    ...
    if (!SdlInputHandler::getUnmappedGamepads().empty()) {
        emitLaunchWarning("An attached gamepad has no mapping and won't be usable.");
    }
    ...
    if ((m_StreamConfig.width > 4096 || m_StreamConfig.height > 4096) && m_Computer->isNvidiaServerSoftware) {
        ...
        if (m_Computer->maxLumaPixelsHEVC == 0 || !(m_Computer->serverCodecModeSupport & SCM_HEVC_MAIN10)) {
            s_streamErrorText = "Your host PC's GPU doesn't support streaming video resolutions over 4K.";
            return false;
        }
        else if ((m_StreamConfig.supportedVideoFormats & ~VIDEO_FORMAT_MASK_H264) == 0) {
            s_streamErrorText = "Video resolutions over 4K are not supported by the H.264 codec.";
            return false;
        }
    }
    ...
    if (m_Preferences->videoDecoderSelection == StreamingPreferences::VDS_FORCE_HARDWARE &&
        ...
        if (m_Preferences->videoCodecConfig == StreamingPreferences::VCC_AUTO) {
            s_streamErrorText = "Your selection to force hardware decoding cannot be satisfied due to missing hardware decoding support on this PC's GPU.";
        }
        else {
            s_streamErrorText = "Your codec selection and force hardware decoding setting are not compatible. This PC's GPU lacks support for decoding your chosen codec.";
        }
...
class DeferredSessionCleanupTask
{
public:
...
//private:
    ...
    void cleanup()
    {
        ...
        //    } catch (const NetworkReplyException&) {
        //    }
...
class AsyncConnectionStartThread
{
public:
    ...
    void start()
    {
        ...
        m_future = std::async(std::launch::async, &Session::startConnectionAsync, m_Session);
    }
    ...
    bool wait(int ms = 0)
    {
        ...
        if(0 == ms)
        {
            m_Session->m_AsyncConnectionSuccess = m_future.get();
            return true;
        }
        else
        {
            std::future_status status = m_future.wait_for(std::chrono::milliseconds(ms));
            if (status == std::future_status::ready)
            {
                m_Session->m_AsyncConnectionSuccess = m_future.get();
                return true;
...
bool Session::startConnectionAsync()
{
    ...
    assert(m_Computer->currentGameId == 0 ||
    ...
    try {
        ...
        std::ostringstream oss;
        oss << "Host returned error: " << e.toString();
        s_streamErrorText = oss.str();
        return false;
    } catch (const NetworkReplyException& e) {
        s_streamErrorText = e.toString();
        return false;
    }

    std::string hostnameStr = m_Computer->activeAddress.address();
    std::string siAppVersion = m_Computer->appVersion;
    ...
    std::string siGfeVersion;
    if (!m_Computer->gfeVersion.empty()) {
        siGfeVersion = m_Computer->gfeVersion;
    }
    if (!siGfeVersion.empty()) {
        hostInfo.serverInfoGfeVersion = siGfeVersion.data();
    }

    // Older GFE and Sunshine versions didn't have this field
    std::string rtspSessionUrlStr;
    if (!rtspSessionUrl.empty()) {
        rtspSessionUrlStr = rtspSessionUrl;
...
public:
    ...
    void start()
    {
        m_future = std::async(std::launch::async, &Session::execInternal, m_Session);
    }
...
void Session::execInternal()
{
    ...
    if (!initialize()) {
        s_state = Session::SessionInitErr;
        Session::releaseSessionControl();
        return;
    }
    ...
    if (!m_AsyncConnectionSuccess) {
        s_state = Session::ConnectionErr;
        delete m_InputHandler;
        m_InputHandler = nullptr;
        SDL_QuitSubSystem(SDL_INIT_VIDEO);
        DeferredSessionCleanupTask cleanup(this);
        cleanup.cleanup();
        return;
    }
...
#else
    std::string windowName = m_Computer->name + " - Remote Play Client";
#endif
    ...
    if (!m_Window) {
        ...
        if (!m_Window) {
            ...
            s_state = Session::SDLWinCreateErr;
            delete m_InputHandler;
            m_InputHandler = nullptr;
            SDL_QuitSubSystem(SDL_INIT_VIDEO);
            DeferredSessionCleanupTask cleanup(this);
        ...
        case SDL_RENDER_TARGETS_RESET:
...

            {
                ...
                if (!chooseDecoder(m_Preferences->videoDecoderSelection,
                    ...
                    s_streamErrorText = "Unable to initialize video decoder. Please check your streaming settings and try again.";
                    s_state = Session::ResetErr;
                    goto DispatchDeferredCleanup;
                }
