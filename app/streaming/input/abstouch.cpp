...
void SdlInputHandler::handleAbsoluteFingerEvent(SDL_TouchFingerEvent* event)
{
    ...
    float vidrelx = (std::min)((std::max)((int)(event->x * windowWidth), dst.x), dst.x + dst.w) - dst.x;
    float vidrely = (std::min)((std::max)((int)(event->y * windowHeight), dst.y), dst.y + dst.h) - dst.y;
...
// #endif
        ...
        result.process_bytes(&event->fingerId, sizeof(event->fingerId));
        pointerId = result.checksum();
    }
    else {
...
void SdlInputHandler::emulateAbsoluteFingerEvent(SDL_TouchFingerEvent* event)
{
    ...
    if (std::sqrt(std::pow(event->x - m_LastTouchDownEvent.x, 2) + std::pow(event->y - m_LastTouchDownEvent.y, 2)) > LONG_PRESS_ACTIVATION_DELTA) {
    ...
    if (event->type != SDL_FINGERDOWN ||
            ...
            std::sqrt(std::pow(event->x - m_LastTouchUpEvent.x, 2) + std::pow(event->y - m_LastTouchUpEvent.y, 2)) > DOUBLE_TAP_DEAD_ZONE_DELTA) {
        // Scale window-relative events to be video-relative and clamp to video region
        short x = (std::min)((std::max)((int)(event->x * windowWidth), dst.x), dst.x + dst.w);
        short y = (std::min)((std::max)((int)(event->y * windowHeight), dst.y), dst.y + dst.h);
