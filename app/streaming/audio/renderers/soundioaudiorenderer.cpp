...
bool SoundIoAudioRenderer::prepareForPlayback(const OPUS_MULTISTREAM_CONFIGURATION* opusConfig)
{
    ...
    packetsToBuffer = std::max(2, packetsToBuffer);
...
void* SoundIoAudioRenderer::getAudioBuffer(int* size)
{
    ...
    *size = std::min(*size, (bytesFree / bytesPerFrame) * bytesPerFrame);
...
void SoundIoAudioRenderer::sioWriteCallback(SoundIoOutStream* stream, int frameCountMin, int frameCountMax)
{
    ...
    frameCountMin = std::max(frameCountMin, (int)(stream->sample_rate * me->m_AudioPacketDuration));
    ...
    frameCountMax = std::min(frameCountMax, (int)(stream->sample_rate * std::max(me->m_AudioPacketDuration * 2, 0.020)));
    frameCountMin = std::min(frameCountMin, frameCountMax);

    // Clamp framesLeft to frameCountMax
    framesLeft = std::min(framesLeft, frameCountMax);
    ...
    for (;;) {
        ...
        frameCount = std::max(framesLeft, frameCountMin);
