...
static int query_callback(int sock, const struct sockaddr *from, size_t addrlen, mdns_entry_type_t entry,
  ...
  mDNS::mdns_out* user_out = static_cast<mDNS::mdns_out* >(user_data);

  if (rtype == MDNS_RECORDTYPE_PTR) {
    mdns_string_t namestr =
        mdns_record_parse_ptr(data, size, record_offset, record_length, namebuffer, sizeof(namebuffer));
    
    //mdns_string_t ptr = deep_copy_mdns_string(&namestr);
    user_out->ptr = convert_to_string(namestr);
    ...
    user_out->port = srv.port;
    user_out->srv_name = convert_to_string(srv.name);
...
std::vector<mDNS::mdns_out> mDNS::executeQuery(const std::string &service) {
  int sockets[32];
  int query_id[32];
  ...
  do {
    ...
    if (res > 0) {
      for (int isock = 0; isock < num_sockets; ++isock) {
        if (FD_ISSET(sockets[isock], &readfs)) {
          mdns_out out;
          records += mdns_query_recv(sockets[isock], buffer, capacity, query_callback, &out, query_id[isock]);
